...
import TodoItem, { TodoList } from "@/types/TodoType";
import TodoApiService from "@/services/todoServices/TodoServices";
...
export const TodoListProvider: React.FC<TodoListProviderProps> = ({ children }) => {
  ...
  /**
   * Toggles the done property of a todo item.
  ...
  const toggleTodoItem = async(id: string) => {
    ...
    if(todo) {
      await todoApiServices.updateTodoState({...todo, done: !todo?.done})
    }
  };
  ...
  const getTodoListProgress = () => {
    ...
    const doneItems = todoList.filter((item) => item.done).length
